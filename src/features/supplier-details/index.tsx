...
import {
  SuppliersProvider,
  useSuppliers,
} from '@/features/suppliers/components/suppliers-provider'
...
function SupplierDetailContent() {
  ...
  const { openPayment } = route.useSearch()
  ...
  const purchasePeriodId = selectedPeriodId
  ...
  // Tự mở popup thanh toán khi điều hướng từ đơn nhập với ?openPayment=true
  const hasAutoOpenedPayment = useRef(false)
  useEffect(() => {
    if (!openPayment || !supplier || hasAutoOpenedPayment.current) return
    hasAutoOpenedPayment.current = true
    setCurrentRow(supplier)
    setOpen('payment')
    // Xoá param để reload không mở lại popup
    navigate({
  ...
  }, [openPayment, supplier, supplierId, navigate, setCurrentRow, setOpen])
