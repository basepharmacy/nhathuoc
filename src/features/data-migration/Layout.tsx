...
import { migrateProducts } from './utils/product-migration'
import { migrateSuppliers } from './utils/supplier-migration'
...
export function DataMigrationLayout() {
  ...
  const handleStartMigration = async () => {
    ...
    try {
      ...
      // Customers - dummy
      if (hasCustomerFile) {
        const customerDummyLogs = dummyLogs.filter((l) =>
          l.message.includes('khách hàng')
        )
        for (const log of customerDummyLogs) {
          addLog(log)
